...
def get_mp3():
	return list(filter(lambda file: re.match('.*\.mp3$', file), os.listdir('/tmp')))[0]

def upload(d, phone_number=0):
	filename = "{0}.mp3".format(str(d['title']))
	filepath = "/tmp/{0}".format(get_mp3())
	client.upload_file(filepath, bucket, filename, ExtraArgs={'ContentType': 'audio/mp3', "Metadata" : {'phoneNumber': phone_number}})
	region = "us-east-1"
	remove_files()
...
def lambda_handler(event, context):
	...
	phone_number = event['Records'][0]['Sns']['MessageAttributes']['phoneNumber']['Value']
	ydl_opts = {
